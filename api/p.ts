...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    res.status(200).send(page(view.title, view.whatItSays ?? "LumiStudio 公开材料", `<main><p class="eyebrow">LumiStudio / Public</p><h1>${escape(view.title)}</h1>${sections}</main>`, `/p/${encodeURIComponent(entry.id)}`));
